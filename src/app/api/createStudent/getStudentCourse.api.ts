...
interface IGetStudentCourseResponse extends AxiosResponse {
  message?: string;
  error?: {
    message: string;
  };
...
export const studentCourseGetApi = async (
): Promise<any> => {
  const token = getCookie("token") ?? "";
//   const id = jwtDecode<DecodedToken>(token)._id;
  const res = (await axiosInstance.get(`/student/getUserCourseList`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  })) satisfies IGetStudentCourseResponse;
  return res.data;
