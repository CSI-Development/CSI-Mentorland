...
function WatchCourse() {
  ...
  useEffect(() => {
    if (!isLoading && data) {
      if (
        ...
        data.sections[0].quiz.length > 0
      ) {
        // setActiveLecture({
        //   url: data.sections[0].lectures[0].url,
        //   description: data.sections[0].lectures[0].description,
        // });
        const updatedActive = [data.sections[0].lectures[0],data.sections[0],data.sections[0].quiz];
  ...
  return (
    <DashboardLayout showSidebar={false}>
      {!isLoading && (
        <div
          className="flex w-full bg-[#f7f9fb] text-black gap-10"
        >
          <div className="w-[70%]">
              <VideoSection
                // @ts-ignore
